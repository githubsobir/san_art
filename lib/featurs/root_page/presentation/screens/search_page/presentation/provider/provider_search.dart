...
class ListApp extends AsyncNotifier<List<SearchEntities>> {
  ...
  Future<void> getSearchApp(int page) async {
    if (state.isLoading) return; // Agar yuklanayotgan bo'lsa, yangi so'rovni boshlama
    try {
      final useCase = ref.read(searchAppUseCase);
      ...
      data.when(
          ...
          state = AsyncValue.data([...currentList, ...success]); // Eski va yangi ma'lumotlarni birlashtirish
        },
            (error) {
